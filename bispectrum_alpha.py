...
bispectra_cpt_even= calculator_bispectrum.calculator_bispectrum('T ', 'even', alpha_bar, Caa_001, CEa_001, CTa_001, CEE, CTE,lmax_alpha, lmax_start, lmax_bis, lmax_computed_cl)
#bispectra_cpt_odd = calculator_bispectrum.calculator_bispectrum('P ', 'odd', alpha_bar, Caa_01, CEa_01, CTa_01, CEE, CTE, lmax_alpha, lmax_start, lmax_bis, lmax_computed_cl)
...
'''
'''
bis_arr = np.transpose(np.loadtxt("./cpt_bispectrum_T_odd_5-1000.dat"))
bispectra_cpt_odd = bis_arr
'''
...
if (alpha_bar!=0):
    for i in range(5):
        ...
        plt.plot(ell, np.abs(ell**4*bispectra_cpt_odd[i+1]),  c=lcs[i], label=blabels[i], lw=3, ls = lts[i])
